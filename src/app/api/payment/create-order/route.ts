...
export async function POST(req: Request) {
  try {
    ...
    console.log('Creating payment order for amount:', body.amount);
    
    const response = await fetch(`${RUST_BACKEND_URL}/api/payment/create-order`, {
      method: 'POST',
      headers: {
    ...
    return NextResponse.json({ error: err?.message || 'Payment order creation failed' }, { status: 500 });
  }
}